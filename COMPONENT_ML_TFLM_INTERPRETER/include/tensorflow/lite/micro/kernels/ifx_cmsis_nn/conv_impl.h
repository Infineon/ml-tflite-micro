...
#ifndef TENSORFLOW_LITE_MICRO_KERNELS_IFX_CMSIS_NN_CONV_IMPL_H_
#define TENSORFLOW_LITE_MICRO_KERNELS_IFX_CMSIS_NN_CONV_IMPL_H_
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  return data.eval_function(context, node, params, data, input, filter,
                                   bias, output, nullptr);
}
...
TfLiteStatus EvalQuantizedPerChannel(
  ...
  if (data.custom_sub8bit_packing) {
      ...
      if (custom->sparsity_coding  != TfliteSparseType::DENSE) {
          int8_t* scratch_buffer = static_cast<int8_t*>(context->GetScratchBuffer(context, data.unpacked_weights_buff_idx));
          ...
          tflite::ops::micro::UnpackSparseWeights(weights_sparse_data,
                                                    weights_size,
                                                    output_depth,
                                                    scratch_buffer);
